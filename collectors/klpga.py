"""
KLPGA (한국여자프로골프) 대회 정보 수집
데이터 소스: schedule_data/klpga_2026.json (4월 초 사용자 제공 예정)
"""
import json
...
SCHEDULE_FILE = Path(__file__).parent.parent / "schedule_data" / "klpga_2026.json"


class KLPGACollector(BaseTournamentCollector):
    TOUR_NAME = "KLPGA"
    ...
    def fetch_schedule(self, reference_date: Optional[datetime] = None, mode: str = "current") -> Optional[TournamentInfo]:
        ...
        print(f"[KLPGA] 검색 주간 ({mode}): {week_start.strftime('%Y-%m-%d')} ~ {week_end.strftime('%Y-%m-%d')}")

        if not SCHEDULE_FILE.exists():
            print(f"[KLPGA] 스케줄 파일 없음 (4월 초 제공 예정): {SCHEDULE_FILE}")
            return None
        ...
        for t in tournaments:
            ...
            if self._is_in_week(start, end, week_start, week_end):
                ...
                return TournamentInfo(
                    ...
                    prize=prize,
                    course=t.get("course", ""),
                    url=t.get("url", "https://www.klpga.co.kr"),
                )
